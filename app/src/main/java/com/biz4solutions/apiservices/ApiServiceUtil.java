...
public class ApiServiceUtil {
    ...
    private String getAuthToken(Context context) {
        return SharedPrefsManager.getInstance().retrieveStringPreference(context, Constants.USER_PREFERENCE, Constants.USER_AUTH_KEY);
    }
                    ...
                    try {
                        if (restClientResponse != null) {
                            if (t instanceof SocketTimeoutException || t instanceof ConnectException) {
                                restClientResponse.onFailure(context.getString(R.string.error_server_connect_error), Constants.INTERNAL_SERVER_ERROR_CODE);
                            } else if (t instanceof JsonSyntaxException) {
                                restClientResponse.onFailure(context.getString(R.string.error_invalid_server_data), Constants.INTERNAL_SERVER_ERROR_CODE);
                            } else {
                                restClientResponse.onFailure(context.getString(R.string.error_internal_server_error) + ", " + t.getMessage(), Constants.INTERNAL_SERVER_ERROR_CODE);
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
